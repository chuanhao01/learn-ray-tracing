...
use rust_simple_raytracer::{
    materials::Dielectric, Camera, CameraParams, Cli, HittableWithBBox, Lambertain, Materials,
    Metal, SolidColor, Sphere, Vec3, BVH,
};
...
fn main() {
    ...
    let cli = Cli::parse();
    cli.save_image(camera.render_rgbimage(&world));
}
